...
# Set File Paths
workspace_data_root = "file:/Workspace/Users/nbaduc137@gmail.com/graph_mining_optimize_flight/data"
tripdelaysFilePath = f"{workspace_data_root}/departuredelays.csv"
airportsnaFilePath = f"{workspace_data_root}/airport-codes-na.txt"
...
# Available IATA codes from the departure delays sample dataset
tripIATA = spark.sql("""
select distinct iata
from (
    select distinct origin as iata from departureDelays
    union all
    select distinct destination as iata from departureDelays
) a
""")
tripIATA.createOrReplaceTempView("tripIATA")

# Only include airports with at least one trip from the dataset
airports = spark.sql("""
select f.IATA, f.City, f.State, f.Country
from airports_na f
